...
+--------------+------------------------------------------------------------------------------------------------------------+
| Opening Data | The date that the station began offering the fuel.                                                         |
+--------------+------------------------------------------------------------------------------------------------------------+
...
For this analysis we'll make use of the tidyverse ecosystem and ggpubr for displaying correlations on plots.
...
```{r clean-pop-density-data, include = FALSE}
 pop_density <- pop_density_raw |> 
    ...
    transmute(state = Name, pop_den = `Resident Population Density`, pop=`Resident Population`) |> 
...
stations <- stations_raw |> 
    ...
    transmute(state = factor(STATE), city = factor(CITY), lat = LATITUDE, lon = LONGITUDE, open_date = ymd_hms(OPEN_DATE), fuel_type = factor(FUEL_TYPE_CODE)) |> 
    # filter by lat/lon for continental US
    filter(lat > 24 & lat < 52, lon < -64 & lon > -126) |> 
    # join with the population data
    inner_join(pop_density, by=c("state" = "abbr")) |> 
...
stations |> 
    ...
    labs(x="Latitude", y="Electric Stations", title="Distribution of Electric Stations by Latitude")
```
...
stations |> 
    ...
    geom_histogram(bins=15) + 
    facet_wrap(vars(fuel_type), scales="free_y") + 
    labs(x="Longitude", y="Stations", title="Distribution of Stations by Longitude")
```
...
stations |> 
    ...
    coord_flip() +
    labs(x="", y="Total Stations", title="Total Number of Stations for Each Fuel Type")
...
station_elec |> 
    ...
    geom_col() + 
    coord_flip() + 
    labs(x="", y="Total Electric Stations", title="Total Electric Stations by City")
```
...
station_elec |> 
    ...
    geom_col(show.legend = FALSE) + 
    coord_flip() + 
    labs(x="", y="Total Number of Stations", title="Total Number of Electric Stations by State")
```
...
station_pop_den |> 
    ggplot(aes(state, station_den, fill=state)) +
    geom_col(show.legend = FALSE) + 
    coord_flip() +
    labs(x="", y="Electric Station Density (station / sq. mi.)", title="Electric Station Density by State")
```
...
station_pop_den |> 
    ...
    labs(y="Electric Station Density (station / sq. mi.)", x = "Population Density (population / sq. mi.)" , title="Do States with Higher Population Densities Have Higher Station Densities?")
```
